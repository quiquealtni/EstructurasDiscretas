...
-- Tipo de dato para representa a las constantes lógicas
-- verdadero y falso .
...
evalua (Syss p q) lista = equiv (evalua p lista) (evalua q lista)
...
-- Función que determina si una fórmula proposicional es tautología dada una lista de  
-- interpretaciones para las variables proposicionales.
      
esTautologia :: Prop -> [Estados] -> Booleano
esTautologia p est = 
    if (elem "Falso" (map show (map (evalua p) est))) == False
       then Verdadero
       else Falso
